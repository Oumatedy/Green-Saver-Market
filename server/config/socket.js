...
const { getOrderStatus, storeMessage, updateOrderStatus } = require('../../backend/utils/socketUtils');
...
const setupSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Join product room with error handling
    ...
    // Join order room with error handling
    socket.on('join-order', async (orderId) => {
      try {
        ...
        const orderStatus = await getOrderStatus(orderId); // You'll need to implement this
    ...
    // Handle user presence with heartbeat
  ...
  setInterval(() => {
    ...
    connectedUsers.forEach((data, userId) => {
      if (now - data.lastActive > 120000) { // 2 minutes
